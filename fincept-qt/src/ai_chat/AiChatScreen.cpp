// AiChatScreen.cpp — Quantum Edge AI Chat, Obsidian design system
...
QWidget* AiChatScreen::build_input_area() {
    ...
    input_box_->setPlaceholderText("Message Quantum Edge AI...");
...
void AiChatScreen::update_stats() {
    ...
    if (llm.is_configured()) {
        ...
        const bool is_fincept = (provider_raw.toLower() == "quantumedge");

        // Display names
        const QString prov_display = is_fincept ? "Quantum Edge AI" : provider_raw.toUpper();
        const QString model_raw = llm.active_model();
        // For fincept, don't expose internal model name
        const QString model_display = is_fincept ? "Quantum Edge AI" : model_raw;
        ...
        model_lbl_->setText(is_fincept ? "Managed by Quantum Edge" : model_short);
        model_lbl_->setToolTip(is_fincept ? "Quantum Edge AI — managed AI service" : model_raw);
        model_lbl_->setStyleSheet(QString("color:%1;font-size:%2px;").arg(col::TEXT_SECONDARY()).arg(fnt::TINY));

        // Header model pill — show "Provider / Model" for clarity
        if (is_fincept) {
            hdr_model_lbl_->setText("Quantum Edge AI");
            hdr_model_lbl_->setToolTip("Quantum Edge managed AI service\n\nChange in Settings > LLM Configuration");
        } else {
